...
function videoView(){
 const current=enriched().slice(0,12).map(x=>{const q=encodeURIComponent(x.name+' IPO review fundamental analysis GMP');return ['Search current IPO','YouTube search — '+x.name,'https://www.youtube.com/results?search_query='+q,'Auto-generated search for the current IPO']});
 const rows=videos.concat(current);
 return '<div class="det-video-grid">'+rows.map(v=>'<a class="det-video" href="'+E(v[2])+'" target="_blank" rel="noopener"><span>'+E(v[0])+'</span><b>'+E(v[1])+'</b><small>'+E(v[3])+'</small>↗</a>').join('')+'</div><div class="det-note">Your supplied videos are kept as research inputs. Additional direct links are included only when a video was discoverable; otherwise the site provides a live YouTube search for each current IPO so new/old analysis can be found without changing the site code.</div>'
}
function render(){
 const el=$('#ipo-decision-content');if(!el)return;
 const opts=enriched();if(!ref&&opts[0])ref=opts[0].name;
 const tabs=[['all','All comparison'],['listing','Listing gain'],['short','Short term'],['long','Long term'],['apply','Application planner'],['videos','Video research']].map(a=>'<button class="det-tab '+(mode===a[0]?'active':'')+'" data-m="'+a[0]+'">'+a[1]+'</button>').join('');
 const body=mode==='videos'?videoView():mode==='apply'?applicationView():rankView(mode==='all'?(rank('short').slice().sort((a,b)=>((b.s.listing+b.s.short+b.s.long)-(a.s.listing+a.s.short+a.s.long)))):rank(mode),mode);
 el.innerHTML='<div class="det-intro"><b>🧠 Decision engine</b><span>Compare Open + Upcoming IPOs for listing-gain, short-term and long-term signals. “Best” is not guaranteed; the panel shows the strongest model signal from the fields currently available.</span></div><div class="det-tabs">'+tabs+'</div><div class="det-controls"><label>IPO for 1–10 lot calculator<select id="det-ref">'+opts.map(x=>'<option value="'+E(x.name)+'" '+(norm(x.name)===norm(ref)?'selected':'')+'>'+E(x.name)+' — '+E(x.board||x.type||'')+'</option>').join('')+'</select></label><label>Optional budget ₹<input id="det-budget" type="number" min="0" step="1000" value="'+(budget||'')+'" placeholder="e.g. 200000"></label><div class="det-presets">'+Array.from({length:10},(_,i)=>'<button type="button" data-x="'+(i+1)+'">'+(i+1)+'× lot</button>').join('')+'</div></div>'+body+'<div class="det-rules"><b>Official rule references:</b> Mainboard retail threshold ₹2L, NII bands above ₹2L/₹10L, and UPI up to ₹5L follow current SEBI/NSE public-issue rules. SME Individual Investor rules are 2+ lots and above ₹2L for issues opening from 1 Jul 2025. Always verify the specific issue's RHP and bid instructions before applying.</div>';
 bind()
}
function bind(){

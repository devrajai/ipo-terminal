...
Pulls the auto-updated data.json produced by the devrajai/ipo-website
repository (Notion IPO Tracker -> GitHub Actions -> data.json) and
regenerates the local data files used by the site:
...
PIPELINE_URL = "https://raw.githubusercontent.com/devrajai/ipo-website/main/data.json"
...
def norm(name):
    ...
    return re.sub(r"limited|ltd|\.|\s+", "", str(name or "").lower())
...
def parse_issue_dates(s):
    ...
    if not s:
        return None, None
    m = re.match(r"(\d{1,2})\s*-\s*(\d{1,2})\s+([A-Za-z]{3})\w*\s+(\d{4})", str(s).strip())
    if not m:
        return None, None

...
function wire(){var nav=document.querySelector('.hbtns');if(!nav)return;['b-brk','b-research-lens','lot1-btn','dna-btn','b-intelligence'].forEach(function(id){var x=document.getElementById(id);if(x)x.remove();});moveFeatures(nav);order(nav);['ipc-btn','ipf-btn','radar-btn'].forEach(function(id){var b=document.getElementById(id);if(b){b.type='button';b.textContent=labels[id];b.dataset.navBound='1';}});if(!nav.dataset.featureDelegate){nav.dataset.featureDelegate='1';nav.addEventListener('click',function(ev){var b=ev.target.closest('.hb');if(!b)return;var key=featureKeyFor(b.id);if(!key)return;ev.preventDefault();ev.stopPropagation();feature(key);},true);}nav.classList.add('ipo-nav-stable');}

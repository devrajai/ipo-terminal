...
function loadScript(src,done){
 var s=document.createElement('script');s.src=src+'?navclick='+Date.now();s.async=false;s.onload=function(){if(done)done();};s.onerror=function(){if(done)done(new Error('Could not load '+src));};document.body.appendChild(s);
}
...
function feature(key){
 ...
 if(p){var b=document.getElementById(id);if(p.classList.contains('show')){p.classList.remove('show');if(b)b.classList.remove('active');return;}openPanel(panel,id);return;}
 var old=document.getElementById(id);if(old)old.remove();
 var tries=0,finished=false;
 loadScript(src,function(err){if(err)showError(err.message);});
 var t=setInterval(function(){var panelEl=document.getElementById(panel),btn=document.getElementById(id);if(panelEl&&btn){clearInterval(t);finished=finishNewFeature(id,panel);return;}if(++tries>=40){clearInterval(t);if(!finished)showError('Panel '+panel+' did not initialize.');}},100);
}
...
function wire(){
 ...
 ['ipc-btn','ipf-btn','dna-btn','radar-btn'].forEach(function(id){var b=document.getElementById(id);if(b){b.type='button';b.textContent=labels[id];b.dataset.navBound='1';}});
 if(!nav.dataset.featureDelegate){
   nav.dataset.featureDelegate='1';
   nav.addEventListener('click',function(ev){
     var b=ev.target.closest('.hb');if(!b)return;
     var key=featureKeyFor(b.id);if(!key)return;
     ev.preventDefault();ev.stopPropagation();feature(key);
   },true);
 }

/* IPO Terminal navigation stabilizer: keeps all feature buttons visible and makes every feature/lot button clickable. */
(function(){
'use strict';
...
function showActive(id){document.querySelectorAll('.lot-decision-btn,.hb').forEach(function(x){if(x.id&&/^((ipf|dna|radar)-btn|lot[123]-btn)$/.test(x.id))x.classList.remove('active');});var b=document.getElementById(id);if(b)b.classList.add('active');}
...
function featureClick(key){var id=modules[key][0],pId=key==='ipf'?'ipo-fundamentals-panel':key==='dna'?'ipo-dna-panel':'ipo-radar-panel',p=document.getElementById(pId);if(p){var b=document.getElementById(id);if(p.classList.contains('show')){p.classList.remove('show');if(b)b.classList.remove('active');}else{p.classList.add('show');showActive(id);}}else rebootFeature(key);}
function openPlanner(lots){var p=document.getElementById('ipo-fundamentals-panel');if(!p){rebootFeature('ipf');setTimeout(function(){openPlanner(lots);},750);return;}if(!p.classList.contains('show')){var b=document.getElementById('ipf-btn');if(b)b.click();}setTimeout(function(){p=document.getElementById('ipo-fundamentals-panel');if(!p)return;p.classList.add('show');var t=document.querySelector('.lot-tabs .planner-tab[data-lots="'+lots+'"]');if(t)t.click();document.querySelectorAll('.lot-decision-btn').forEach(function(x){x.classList.remove('active');});var b=document.getElementById('lot'+lots+'-btn');if(b)b.classList.add('active');p.scrollIntoView({behavior:'smooth',block:'start'});},350);}
function wire(){
 ...
 [['lot1-btn',1],['lot2-btn',2],['lot3-btn',3]].forEach(function(x){var b=document.getElementById(x[0]);if(b&&!b.dataset.navBound){b.dataset.navBound='1';b.textContent=labels[b.id];b.onclick=function(){openPlanner(x[1]);};}});

(function(){
  ...
  /* Strong, self-contained liquid-glass UI. This intentionally lives in the
     final JS so older CSS rules cannot disable the visual treatment. */
  ...
  css.textContent=`
    ...
    body.light{--lg-bg:#eef2f7;--lg-panel:rgba(255,255,255,.55);--lg-panel2:rgba(255,255,255,.40);--lg-border:rgba(60,70,85,.20);--lg-text:#182131;--lg-sub:#566274}
    body.light{background:radial-gradient(700px 420px at 15% 8%,rgba(140,160,190,.24),transparent 62%),radial-gradient(620px 420px at 88% 18%,rgba(100,125,230,.16),transparent 62%),linear-gradient(180deg,#f7f9fc,#e9edf3 72%)!important}
    .hdr,.kpi-row,.hbtns,.panel,.panel-title,.ipo-card,.ipo-stats div,.gl-card,.broker-card,.broker-grid div,.gloss-card,.tip-tab,.allot-wrap,.news-item,.doc-row,.ipo-table th,.theme-btn,.back-top{
    ...
    .hbtns{position:sticky!important;top:0!important;z-index:900!important;transition:transform .24s ease,opacity .24s ease,max-height .24s ease,padding .24s ease!important;overflow-x:auto!important;overflow-y:hidden!important;max-height:190px!important}
    ...
    .hbtns.nav-collapsed:not(.nav-hidden){max-height:190px!important}
    ...
    .panel{scroll-margin-top:18px!important}
    .panel.show{animation:lg-panel-in .20s ease both!important}
    ...
    *{scrollbar-color:rgba(255,255,255,.20) transparent}
    body.light *{scrollbar-color:rgba(50,60,75,.20) transparent}
  `;
  ...
  function themeSync(){
    ...
    if(b){b.textContent=light?'☀️':'🌙';b.setAttribute('aria-label',light?'Switch to dark mode':'Switch to light mode');b.title=light?'Switch to dark mode':'Switch to light mode';}
  }
  ...
  function closeAllExcept(panel){
    document.querySelectorAll('.panel.show').forEach(function(p){if(p!==panel)p.classList.remove('show');});
    document.querySelectorAll('.hb.active').forEach(function(b){if(!panel || b.getAttribute('data-panel')!==panel.id)b.classList.remove('active');});
  }
  function togglePanel(btn){
    ...
    if(was){panel.classList.remove('show');btn.classList.remove('active');body.classList.remove('panel-open');}
    else{panel.classList.add('show');btn.classList.add('active');body.classList.add('panel-open');}
    var nav=document.querySelector('.hbtns');if(nav){nav.classList.add('nav-collapsed');}
  }

  /* Direct capture handlers beat the legacy page handlers and work with touch. */
  var theme=document.getElementById('themeToggle');
  if(theme){
    theme.addEventListener('pointerdown',function(e){e.preventDefault();e.stopPropagation();e.stopImmediatePropagation();setTheme(!body.classList.contains('light'));},{capture:true});
    theme.addEventListener('click',function(e){e.preventDefault();e.stopPropagation();e.stopImmediatePropagation();},{capture:true});
  }
  document.querySelectorAll('.hb').forEach(function(btn){
    btn.addEventListener('pointerdown',function(e){
      if(e.pointerType==='mouse' && e.button!==0)return;
      e.preventDefault();e.stopPropagation();e.stopImmediatePropagation();togglePanel(btn);
    },{capture:true});
    btn.addEventListener('click',function(e){e.preventDefault();e.stopPropagation();e.stopImmediatePropagation();},{capture:true});
  });

  /* Close button must also reliably collapse the navigation state. */
  document.addEventListener('pointerdown',function(e){
    var x=e.target.closest&&e.target.closest('.close-x');
    if(!x)return;
  ...
  window.addEventListener('scroll',function(){
    ...
    requestAnimationFrame(function(){
      ...
      lastY=window.scrollY||0;ticking=false;
    });
  },{passive:true});
